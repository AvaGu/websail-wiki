...
import edu.northwestern.websail.el.client.api.model.feature.FeatureMap;
import edu.northwestern.websail.el.client.api.model.feature.FeatureMapSet;
...
public class WebSAILWikifierAPIAdapter {
	...
	private static final String serviceRoot = "http://chandra.cs.northwestern.edu:8080/wikifier";
	...
	public String post(String api, Object obj) throws ClientProtocolException, IOException{
		String endPoint = serviceRoot + "/" + api;
...
//	}
	...
	public FeatureMapSet getMentionFeature(String mentionKey, String groupName)
			throws IOException {
		String api = "feature/get/" + mentionKey + "/" + groupName;
		InputStream json = this.getInputStream(api);
		Gson gson = new Gson();
		Type type = new TypeToken<Response<FeatureMapSet>>() {
		}.getType();
		BufferedReader br = new BufferedReader(new InputStreamReader(json));
		Response<FeatureMapSet> response = gson.fromJson(br, type);
		br.close();
		return response.getResponse();
	}

	public boolean uploadMentionFeature(FeatureMapSet fms) throws ClientProtocolException, IOException {
		String res = this.post("feature/post",fms);
		return res.equalsIgnoreCase("{\"status\":200,\"response\":\"ok\",\"message\":\"OK\"}");
